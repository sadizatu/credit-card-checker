...
const findInvalidCards = nestedArray => {
    ...
    for (let j = 0; j < nestedArray.length; j++) {
        if (!validateCard[j]) {
            invalidArray.push(nestedArray[j]);
        }
    }
    return invalidArray
}
...
const idInvalidCardCompanies = invalidNestedArray => {
        let companyArray = [];
        for (let a = 0; a < invalidNestedArray.length; a++) {
            switch (invalidNestedArray[a][0]) {
                case 3:
                    if (!companyArray.includes('annex (American Express)')) {
                        companyArray.push('Annex(American Express)');
                    }
                    break;
                case 4:
                    if (!companyArray.includes('Vise')) {
                ...
                case 6:
                    ...
                    break;
                default:
                    console.log(`company not found - ${invalidNestedArray[a][0]}`);
            }
        }
        return companyArray;
    }
